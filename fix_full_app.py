...
<head>
    ...
    <style>
        ...
        .chat-box { height: 350px; border: 1px solid var(--border); border-radius: 8px; padding: 15px; overflow-y: scroll; background: #fafafa; margin-bottom: 15px; display: flex; flex-direction: column; gap: 10px; }
        .message { padding: 10px 14px; border-radius: 8px; max-width: 80%; font-size: 14px; line-height: 1.5; }
        .user-msg { background: var(--primary); color: white; align-self: flex-end; }
        .bot-msg { background: #e9f5ed; color: var(--text); align-self: flex-start; border: 1px solid #d0e8d9; }
        .chat-input-area { display: flex; gap: 10px; }
        .chat-input { flex: 1; padding: 12px; border: 1px solid var(--border); border-radius: 8px; font-size: 14px; }
        .send-btn { background: var(--primary); color: white; border: none; padding: 0 20px; border-radius: 8px; font-weight: 600; cursor: pointer; }
    </style>
</head>
<body>
    <div class="container">
        
        <!-- 1. Scholar Profile Section -->
        <div class="card">
        ...
        <!-- 2. Main AI Chat Interface Section -->
        <div class="card">
            <h2 style="margin-top:0; color: var(--primary);">Tibyan AI Assistant 💬</h2>
            <div class="chat-box" id="chatBox">
                <div class="message bot-msg">Assalamu Alaikum! Main Tibyan AI hoon. Aap apni koi bhi Islamic query ya sawal yahan pooch sakte hain.</div>
            </div>
            <div class="chat-input-area">
                <input type="text" id="userInput" class="chat-input" placeholder="Type your question here..." onkeypress="handleKeyPress(event)">
    ...
    <script>
        ...
        function sendMessage() {
            ...
            // Simulate AI response (or connect to backend route if available)
            setTimeout(() => {
                const botDiv = document.createElement('div');
                botDiv.className = 'message bot-msg';
                botDiv.innerText = "Aapke sawal '" + text + "' par scholarly analysis taiyar ki ja rahi hai. (Authenticated response)";
